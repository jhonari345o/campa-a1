...
type Asset = { file: File; url: string; width: number; height: number; durationSeconds: number | null };
...
export function CreativeLab() {
  ...
  const review = useMemo(() => asset ? reviewCreativeAsset({ placement, mimeType: asset.file.type, sizeBytes: asset.file.size, width: asset.width, height: asset.height, durationSeconds: asset.durationSeconds, hasRights: rights, hasSound: sound, hasCaptions: captions, cta }) : null, [asset, captions, cta, placement, rights, sound]);

  async function chooseFile(file?: File) {
    if (!file) return;
    if (!file.type.startsWith("image/") && !file.type.startsWith("video/")) { setError("Selecciona una imagen o video."); return; }
    if (file.size > 250 * 1024 * 1024) { setError("Para revisión local, usa un archivo menor a 250 MB."); return; }
    setError("");
    if (asset?.url) URL.revokeObjectURL(asset.url);
    const url = URL.createObjectURL(file);
    try {
      const metadata = file.type.startsWith("video/") ? await videoMetadata(url) : await imageMetadata(url);
      setAsset({ file, url, ...metadata });
    } catch { URL.revokeObjectURL(url); setError("No pude leer el archivo. Prueba una versión MP4, JPG, PNG o WebP."); }
  }
  ...
  return <div className="space-y-6">
    <section className="overflow-hidden rounded-panel border border-border bg-white shadow-panel"><header className="bg-forest p-7 text-white"><p className="text-xs font-black uppercase tracking-[.2em] text-signal">Laboratorio creativo</p><h1 className="mt-2 text-3xl font-black">Revisa la pieza antes de pagar por mostrarla.</h1><p className="mt-2 max-w-3xl text-sm text-white/60">El archivo se inspecciona localmente en tu navegador. No se sube a nuestros servidores ni al proveedor de IA; Mavi recibe únicamente el diagnóstico y el texto que escribas.</p></header><div className="grid gap-6 p-6 lg:grid-cols-[.9fr_1.1fr]"><div className="space-y-4"><label className="block text-xs font-black uppercase text-forest">Ubicación publicitaria<select value={placement} onChange={(event) => setPlacement(event.target.value as CreativePlacement)} className="mt-1 min-h-11 w-full rounded-xl border border-border bg-fog px-3 text-sm font-normal normal-case">{PLACEMENTS.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label className="grid min-h-36 cursor-pointer place-items-center rounded-2xl border-2 border-dashed border-signal/40 bg-signal/5 p-6 text-center"><input type="file" accept="image/*,video/*" className="sr-only" onChange={(event) => void chooseFile(event.target.files?.[0])} /><span><strong className="block text-forest">Seleccionar imagen o video</strong><small className="mt-1 block text-muted">JPG, PNG, WebP, MP4 o formato compatible con tu navegador</small></span></label><div className="grid gap-3 sm:grid-cols-2"><Toggle checked={rights} onChange={setRights} label="Derechos comerciales confirmados" /><Toggle checked={sound} onChange={setSound} label="Tiene audio o voz" /><Toggle checked={captions} onChange={setCaptions} label="Tiene subtítulos" /><label className="text-xs font-black text-forest">CTA principal<input value={cta} onChange={(event) => setCta(event.target.value)} maxLength={80} placeholder="Ej. Comprar ahora" className="mt-1 h-11 w-full rounded-xl border border-border bg-fog px-3 text-sm font-normal" /></label></div><label className="block text-xs font-black text-forest">Copy o mensaje principal<textarea value={copy} onChange={(event) => setCopy(event.target.value)} maxLength={1200} rows={4} className="mt-1 w-full rounded-xl border border-border bg-fog p-3 text-sm font-normal" placeholder="Pega aquí el texto de la pieza." /></label></div><div>{asset ? <CreativePreview asset={asset} placement={placement} /> : <div className="grid min-h-80 place-items-center rounded-2xl bg-fog p-8 text-center"><div><span className="text-5xl">✦</span><strong className="mt-3 block text-lg text-forest">Tu pieza aparecerá aquí</strong><p className="mt-1 text-sm text-muted">Mostraremos formato, resolución, duración y una zona segura orientativa.</p></div></div>}</div></div></section>
...
function CreativePreview({ asset, placement }: { asset: Asset; placement: CreativePlacement }) { const vertical = placement === "meta_reels" || placement === "youtube_shorts"; return <div><div className={`relative mx-auto overflow-hidden rounded-2xl bg-black ${vertical ? "aspect-[9/16] max-h-[560px]" : placement === "meta_feed" ? "aspect-[4/5] max-h-[520px]" : "aspect-[16/7]"}`}>{asset.file.type.startsWith("video/") ? <video src={asset.url} controls muted className="size-full object-contain" /> : <Image src={asset.url} alt="Vista previa del creativo" width={asset.width} height={asset.height} unoptimized className="size-full object-contain" />}<div className="pointer-events-none absolute inset-[8%] rounded-xl border-2 border-dashed border-signal/80"><span className="absolute left-2 top-2 rounded bg-black/65 px-2 py-1 text-[9px] font-black text-signal">ZONA SEGURA ORIENTATIVA</span></div></div><div className="mt-3 flex flex-wrap justify-center gap-2 text-[10px] font-bold text-muted"><span>{asset.width} × {asset.height}px</span><span>•</span><span>{(asset.file.size / 1024 / 1024).toFixed(1)} MB</span>{asset.durationSeconds != null && <><span>•</span><span>{asset.durationSeconds.toFixed(1)} s</span></>}</div></div>; }
...
function videoMetadata(url: string) { return new Promise<{ width: number; height: number; durationSeconds: number }>((resolve, reject) => { const video = document.createElement("video"); video.preload = "metadata"; video.onloadedmetadata = () => resolve({ width: video.videoWidth, height: video.videoHeight, durationSeconds: Number.isFinite(video.duration) ? video.duration : 0 }); video.onerror = reject; video.src = url; }); }
