...
5. Ejecutar en orden las migraciones de `supabase/migrations`, incluidas
   `0006_payphone_meta_pauta.sql` y `0010_dlocal_go_checkout.sql`, antes de probar el flujo de cobro.
...
## Cobro dLocal Go y pauta Meta

El sitio no captura tarjetas. Crea un Checkout estándar alojado por dLocal Go y
solo reconoce el pago después de validar el webhook HMAC y consultar la API
oficial para comparar orden, monto exacto, USD, país y estado. La inversión base,
el cargo del 22% y la comisión del 25% se registran por separado.

dLocal Go liquida el cobro a Ad Mavericks; no paga automáticamente la factura de
Meta. Meta consume únicamente la inversión base y la factura mediante el método
de pago configurado en la cuenta publicitaria. Un platform admin crea primero la
campaña completa en `PAUSED`; iniciar gasto es una acción posterior y confirmada.
Configuración completa en [`docs/DLOCAL_META.md`](docs/DLOCAL_META.md).
