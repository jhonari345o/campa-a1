...
export function MaviFloatingAssistant() {
  ...
  const campaignHref = useMemo(() => {
    ...
    if (postUrl) params.set("post", postUrl);
    return `/pautar?${params.toString()}`;
  }, [network, budget, objective, postUrl]);
    ...
    try {
      ...
      if (!response.ok) setError(data.error ?? "Mavi no pudo responder.");
        ...
        if (data.action?.kind === "prepare_campaign") {
          setNetwork(data.action.network);
          if (data.action.budget) setBudget(String(data.action.budget));
          setObjective(data.action.objective);
          if (data.action.postUrl) setPostUrl(data.action.postUrl);
          setMode("campaign");
        }
      }
    } catch {
